...
// Example: const TopDestinationsChart = dynamic(() => import('@/components/charts/TopDestinationsChart'), { ssr: false });

const TopDestinationsChart = ({ data }) => {
  // Filter out entries with missing or invalid count or name
  ...
  if (safeData.length === 1) {
    return (
      <div style={{ textAlign: "center", padding: "2rem" }}>
        Only one destination: <b>{safeData[0].name}</b>
      </div>
    );
  }
